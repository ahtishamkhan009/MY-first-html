             ...
             <div class="nav-bar">
                <div class="container-fluid">
                    <nav class="navbar navbar-expand-lg bg-dark navbar-dark">
                        <a href="#" class="navbar-brand" style="height: auto; width:50%;">
                            <img src="img/logo1.png" alt="Logo" style="height: auto; width: 100%;">
                        </a>
...
<script>
    // Function to set default text in the iframe body
    function setDefaultText() {
        // Get the iframe element
        var iframe = document.getElementById('editor-richText-box');
		const fileName = fileName;

		// Construct the path to the file based on the current script's location
		const filePath = window.location.href.substring(0, window.location.href.lastIndexOf("/")) + "/" + fileName;
		...
		fetch(filePath)
			...
			.then(fileContent => {
				// Set the content of the iframe directly
				const iframe = document.getElementById('editor-richText-box');
				if (iframe.contentDocument) {
					const bodyElement = iframe.contentDocument.body;
					bodyElement.id = 'myIframeBody';

					// Inserting HTML content to the body
					bodyElement.insertAdjacentHTML('beforeend', fileContent);
				} else {
					// If the iframe is not yet loaded, handle accordingly
					console.error('Iframe is not yet loaded.');
				}
			})
			.catch(error => {
        ...
        // if (iframe.contentDocument) {
        //     // Access the body of the iframe and set default text
        //     iframe.contentDocument.body.innerHTML ="";
        // } else {
        //     // If the iframe is not yet loaded, try again after a short delay
        //     setTimeout(setDefaultText, 100);
        // }
    }
...
<script>
	function downloadPDF() {
			...
			console.log("working");
            const iframe = document.getElementById('editor-richText-box');
    		const bodyElement = iframe.contentDocument.getElementById('myIframeBody');
			var doc = new DOMParser().parseFromString(bodyElement.innerHTML, 'text/html');
            ...
            input.value = doc.body.textContent || "";
